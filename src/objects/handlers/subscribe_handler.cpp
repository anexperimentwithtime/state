...
namespace aewt::handlers {
    void subscribe_handler(const boost::uuids::uuid transaction_id, const std::shared_ptr<response> &response,
                   const std::shared_ptr<state> &state, const std::shared_ptr<session> &session,
                   const boost::json::object &data) {
        if (validators::subscriptions_validator(transaction_id, response, data)) {
            ...
            const bool _success = state->subscribe(session->get_id(), _client_id, _channel);
