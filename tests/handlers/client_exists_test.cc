...
TEST(handlers_client_exists_test, can_handle_yes) {
    ...
    auto _client_exists_transaction_id = to_string(_state->get_generator()());

    _state->add_client(boost::lexical_cast<boost::uuids::uuid>(_client_id), _session->get_id());

    const boost::json::object _data = {{"action", "client_exists"}, {"transaction_id", _client_exists_transaction_id}, {"params", {{"client_id", _client_id}, {"session_id", to_string(_session->get_id())}}}};
    ...
    ASSERT_EQ(_response->get_data().at("transaction_id").as_string(), _client_exists_transaction_id);
}

TEST(handlers_client_exists_test, can_handle_no) {
    const auto _state = std::make_shared<aewt::state>();
    ...
    ASSERT_TRUE(_response->get_data().contains("message"));
    ASSERT_TRUE(_response->get_data().at("message").is_string());
    ASSERT_EQ(_response->get_data().at("message").as_string(), "no");
    ASSERT_TRUE(_response->get_data().contains("data"));
    ASSERT_TRUE(_response->get_data().at("data").is_object());
