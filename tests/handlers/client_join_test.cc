...
TEST(handlers_client_join_test, can_handle) {
    ...
    const boost::json::object _data = {{"action", "client_join"}, {"transaction_id", _client_join_transaction_id}, {"params", {{"client_id", _client_id}, {"session_id", to_string(_session->get_id())}}}};
    ...
    ASSERT_EQ(_response->get_data().at("transaction_id").as_string(), _client_join_transaction_id);
}

TEST(handlers_client_join_test, can_handle_no_effect) {
    const auto _state = std::make_shared<aewt::state>();
