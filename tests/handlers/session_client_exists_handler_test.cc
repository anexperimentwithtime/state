...
TEST(handlers_session_client_exists_handler_test, can_handle) {
    ...
    const boost::json::object _data = {{"action", "session_client_exists"}, {"transaction_id", _session_client_exists_transaction_id}, {"params", {{"client_id", _client_id}, {"session_id", to_string(_session->get_id())}}}};
    ...
    ASSERT_EQ(_response->get_data().at("transaction_id").as_string(), _session_client_exists_transaction_id);
}