...
TEST(handlers_session_test, can_handle) {
    ...
    const boost::json::object _data = {{"action", "session"}, {"transaction_id", _transaction_id}, {"params", boost::json::object{{"session_id",to_string(_session->get_id()) } } } };
...
TEST(handlers_session_test, can_handle_no_effect) {
    const auto _state = std::make_shared<aewt::state>();
    ...
    const boost::json::object _data = {{"action", "session"}, {"transaction_id", _transaction_id}, {"params", {{"session_id", to_string(_session->get_id())}}}};
