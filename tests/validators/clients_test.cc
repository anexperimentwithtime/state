...
TEST(validators_clients_test, can_handle_empty_params_client_id_on_clients) {
    ...
    for (const auto _action : {"client_join", "client_leave"}) {
        ...
        const boost::json::object _data = {{"action", _action}, {"transaction_id", _transaction_id}, {"params", {{"session_id", to_string(_session->get_id())}}}};
...
TEST(validators_clients_test, can_handle_wrong_params_client_id_primivite_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
    for (const auto _action : {"client_join", "client_leave"}) {
        auto _transaction_id = to_string(_state->get_generator()());
        const boost::json::object _data = {{"action", _action}, {"transaction_id", _transaction_id}, {"params", {{"session_id", to_string(_session->get_id())}, {"client_id", 7}}}};
...
TEST(validators_clients_test, can_handle_wrong_params_client_id_type_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
...
TEST(validators_clients_test, can_handle_empty_params_session_id_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
    for (const auto _action : {"client_join", "client_leave"}) {
        ...
        const boost::json::object _data = {{"action", _action}, {"transaction_id", _transaction_id}, {"params", {{"client_id", _client_id}}}};
...
TEST(validators_clients_test, can_handle_wrong_params_session_id_primivite_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
    for (const auto _action : {"client_join", "client_leave"}) {
        auto _transaction_id = to_string(_state->get_generator()());
        auto _client_id = to_string(_state->get_generator()());
        const boost::json::object _data = {{"action", _action}, {"transaction_id", _transaction_id}, {"params", {{"session_id", 7}, {"client_id", _client_id}}}};
        const auto _response = kernel(_state, _session, _data);
...
TEST(validators_clients_test, can_handle_wrong_params_session_id_type_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
...
TEST(validators_clients_test, can_handle_empty_params_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
...
TEST(validators_clients_test, can_handle_wrong_params_primivite_on_clients) {
    const auto _state = std::make_shared<aewt::state>();
    boost::asio::io_context _io_context;
    boost::asio::ip::tcp::socket _socket(_io_context);
    const auto _session = std::make_shared<aewt::session>(boost::uuids::random_generator()(), std::move(_socket));
