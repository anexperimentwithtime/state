...
TEST(kernel_test, can_handle_non_valid_data) {
  ...
  ASSERT_EQ(_response->get_data().at("message").as_string(),
  ...
  ASSERT_TRUE(
      _response->get_data().at("data").as_object().at("action").is_string());
  ASSERT_EQ(
      _response->get_data().at("data").as_object().at("action").as_string(),
...
TEST(kernel_test, can_handle_pong) {
  ...
  ASSERT_TRUE(_response->get_data().at("data").is_object());
  ASSERT_TRUE(
      _response->get_data().at("data").as_object().contains("timestamp"));
  ASSERT_TRUE(
      _response->get_data().at("data").as_object().at("timestamp").is_number());
  ASSERT_TRUE(
      _response->get_data().at("data").as_object().at("timestamp").as_int64() >
      0);
  ASSERT_TRUE(
      _response->get_data().at("data").as_object().at("timestamp").as_int64() <
